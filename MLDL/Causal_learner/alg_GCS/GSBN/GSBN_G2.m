...
function [G,time,test] = GSBN_G2(Data,alpha,ns,p,maxK)
%
% GSBN_G2 learns a causal graph on discrete data
%
% INPUT :
...
for i=1:p
    ...
    MB=GS_G2(Data,i,alpha,ns,p,maxK);
    PP(i,MB)=1;
        ...
        for k=1:length(SZ)
            ...
            while length(B) >= cutSetSize&&cutSetSize<=maxK
                ...
                for si=1:length(SS)
                    S = SS{si};
                    test=test+1;
                    [pval]=my_g2_test(Y,Z,myunion(S,X),Data,ns,alpha);    
                    if isnan(pval)        
                        CI=0;
                    else
                        if pval<=alpha
