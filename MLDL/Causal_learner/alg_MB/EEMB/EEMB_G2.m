...
function [MB,test,time] = EEMB_G2(Data,target,alpha,ns,p,maxK)
%
% EEMB_G2 finds the Markov blanket of target node on discrete data
%
% INPUT :
            ...
            for si=1:length(SS)
                ...
                if CI==1 
                    ...
                    for o=1:length(CanPC)
                        ...
                        test = test +1;
                        [pval,dep_sp(pc_var,Y)]=my_g2_test(target,Y,S,Data,ns,alpha);
                        if isnan(pval)
                            CI=1;                      
                        else
                            if pval<=alpha
