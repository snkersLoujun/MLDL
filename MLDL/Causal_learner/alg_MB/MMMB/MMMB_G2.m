...
function [MB,test,time] = MMMB_G2(Data,target,alpha,ns,p,maxK)
%
% MMMB_G2 finds the Markov blanket of target node on discrete data
%
% INPUT :
...
[pc,ntest1,~,sepset]=MMPC_G2(Data,target,alpha, ns, p, maxK);
 test=test+ntest1;
 MB=[MB pc];
 
for i=1:length(pc)

    [pc_tmp,ntest2]=MMPC_G2(Data,pc(i),alpha, ns, p, maxK);
     test=test+ntest2;
